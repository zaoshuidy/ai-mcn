...
```text
当前阶段：Stage 3 - 真实10位达人采集与评分（v2 评分校准完成，≥85 真实候选 2/10、Top3 门槛仅 1 位达标，blocked_by_insufficient_verified_candidates，待项目总控决策）
```
...
| Stage 2 - 达人搜索策略与候选采集 | 已通过项目总控验收（96/100）；四项放行条件全部满足；基础设施已冻结 |
| Stage 3 - 真实10位达人采集与评分 | v2 校准后部分完成：候选池与风格研究对象门禁通过（欧盈Kelly v2 93 分恢复为 research_style_reference）；**最终候选 2/10、Top3 门槛仅 1 位达标**（10 维 v2 模型 24 位重评分后 ≥85 仅欧盈Kelly 93、小季没烦恼 85 两位真实候选，≥90 仅 1 位，不虚构不降级纳入低分账号），`stage_3_final_10_ready=false`、`stage_3_top3_ready=false`，等待项目总控决策是否追加搜索预算 |

> Stage 3 已完成（v2 校准口径）：8 关键词搜索 113 位真实原始候选、预筛 30 位、深核补足至 24 位（gapfill 20 位来源、新深核 9 位、新淘汰 4 位）、240 篇互动数据、24 位 10 维 v2 可复现评分、Top3 门槛结果 1 位（欧盈Kelly 93，唯一 ≥90）、Top3 代表视频完整理解管线、18 张截图证据；脚本风格研究对象为欧盈Kelly（research_style_reference，非商业定案）。v1 结果（欧盈Kelly 95 / 小季没烦恼 91 / 一只牛🐮 90）已标记 superseded 仅供追溯；一只牛🐮（139 粉丝）v2 降至 80 并标记 koc_seed_candidate，不得进入 Top3。
> 全部候选 human_verified=false；评分函数可复现；无 Cookie/xsec_token 入库。达人风格Skill、脚本生成、飞书接入等功能**尚未实现**，将在后续阶段按门禁逐步推进。
...
- **Stage 2 第四轮（2026-07-18）：代表性生活 Vlog 端到端理解 POC 完成**——从专用 CDP 会话真实搜索「早餐vlog」（22 条结果/20 条视频），按硬性规则筛选（时长 30—120s、180 天内、非品牌官方号、食品生活场景、排除极端身材表达），第 4 条候选入选：曲琪小饼干儿《今日早餐🥣酸汤麻酱面➕虾饺➕小烤肠》（note_id=6a22183c000000002202364e，71.7s，720x1280，14,122,005 bytes，页面/文件时长漂移 0.003s）；faster-whisper small 转写 18 片段并**如实标注为 BGM 误识别**（视频无有效口播，信息载体为连续操作字幕，覆盖 5/6 片段）；每 2s 抽帧 36 张+场景切换 20 张，去重后 41 张，29 张有效间隔帧全部多模态读取；产出 6 阶段音视频联合时间线与 15 问风格拆解报告（`reports/stage_2_representative_video_analysis.md`）；无商业植入信号、无合规风险表达；代表性门禁评估模块（`src/representative_poc.py`）+ 21 项离线测试 + Stage 2 验证器扩展；**双层 POC 定义落地**（minimal=链路可运行 / representative=真实 Vlog 可分析），Stage 2 基础设施按任务书冻结；
- 视频理解管线（获取→转写→抽帧→时间线→分析）代码就绪并经两轮真实视频验证；yt-dlp 两轮真实执行均失败（图文笔记无视频；未登录页面不含视频流），XHS-Downloader 需 Cookie 属策略禁止；6 个 CLI/包工具登记 poc_required（CAND-005~010）。

## 16. 当前未完成内容

- **Stage 3 最终 10 位定案未达成（v2 后 2/10）**：10 维 v2 模型完成 24 位深核重评分与补足后，≥85 真实候选仅 2 位（欧盈Kelly 93、小季没烦恼 85），≥90 仅 1 位，Top3 门槛无法凑满（不虚构不降级）；结构平衡缺口为健身 0/2、字幕 1/3、口播 1/3；`stage_3_final_10_ready=false`、`stage_3_top3_ready=false`，是否追加搜索预算由项目总控决策；
- 视觉类字段标注（除已完成管线的 4 条 POC/Top3 视频外）：屏幕字幕/动作/场景/镜头/产品露出仍需逐条处理；
- 最终 10 位达人定案与商业合作定案（未做，Stage 3 仅产出研究候选）；
- 达人内容风格深度拆解（Stage 4，含风格研究对象三视频拆解）；
...
Stage 3 候选池、Top3 与风格研究对象门禁已通过，但最终 10 位定案因真实达标候选不足（6/10）而阻塞。**下一阶段为 Stage 4：达人内容风格拆解**——对风格研究对象（欧盈Kelly）的 3 条真实视频做深度拆解；正式放行须项目总控先决策：追加搜索预算补足 10 位，或确认以 6 位真实研究候选推进。
