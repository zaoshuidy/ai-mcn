...
| references/rules.md | 中文短视频自然化规则 R1 ~ R8（自研）与红线 |
| references/examples.md | 一段 AI 腔轻醒酸奶脚本 → 自然化版本的完整对照（原创示例） |
| references/output-schema.md | 输出契约逐条说明（与校验器一一对应） |
| evals/evals.json | 3 条真实场景 eval + 1 条反例（事实被篡改必须判失败） |
| scripts/validate_output.py | 输出契约校验器（schema + 事实保留 + 评分一致性） |
