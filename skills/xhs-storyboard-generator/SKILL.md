...
- `rules.md`：可执行性铁律、运镜黑名单、时长证据、合规红线、道具与场景约束。
- `examples.md`：早餐场景脚本 → 9 镜完整分镜示例（可直接当模板）。
- `evals.json`：3 个真实 eval（2 正例 + 1 反校验例）+ 1 个不可执行运镜反例。
- `validate_output.py`：输出校验器（schema / 时间轴 / 总时长 / 可执行性 / 产品露出时点）。
